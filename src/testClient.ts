...
function send(proc: ChildProcess, obj: Json) {
  const body = JSON.stringify(obj);
  const msg = `Content-Length: ${Buffer.byteLength(body, "utf8")}\r\n\r\n${body}`; // <- CRLF
  if (!proc.stdin) return console.error("Error: proc.stdin is null");
  proc.stdin.write(msg);
}

function attachReader(proc: ChildProcess, onMessage: (msg: Json) => void) {
  ...
  proc.stdout?.on("data", (chunk: Buffer) => {
    ...
    while (true) {
      ...
      if (headerEndCRLF === -1) break; // wait for full header
      ...
      if (buf.length < bodyEnd) break; // wait for full body
...
function sendInitialize(proc: ChildProcess, id = 1) {
  send(proc, {
    jsonrpc: "2.0",
    id,
    method: "initialize",
...
function sendInitialized(proc: ChildProcess) {
  send(proc, { jsonrpc: "2.0", method: "notifications/initialized", params: {} });
}

function callAdd(proc: ChildProcess, id = 2, a = 2, b = 3) {
...
// Hard kill fallback
setTimeout(() => { try { server.kill(); } catch {} }, 9000);
